...
async function loadGachaRecords(uid) {
  ...
  if (!filteredRecords.length) {
    container.innerHTML =
      "<p>没有调频记录。请先打开游戏祈愿界面，然后点击刷新数据</p>";
    return;
  }

  // 初始化ZZZ星级映射系统
  try {
    const analysis = initializeZzzMapping(filteredRecords);
    console.log("[ZZZ数据加载] 星级映射:", analysis.details);

    // 在界面中显示数据格式信息（可选）
    if (analysis.uniqueRanks.length < 3) {
      console.warn("[ZZZ数据警告] 只有部分星级数据:", analysis.details);
    }
  } catch (error) {
    container.innerHTML = `<p>ZZZ数据格式错误: ${error.message}</p>`;
    console.error("[ZZZ数据加载失败]", error);
    return;
  }
  ...
  const GACHA_TYPE_ORDER = ["独家频段", "音擎频段", "常驻频段", "邦布频段"];

  console.log("1");
  const safeValue = (value, fallback = "无数据") =>
  ...
  const generateRatingCards = (records, poolType) => {
    const fiveStarAvg = calculateDrawsBetweenZzz(records, "S");
    const isCharacterOrConeEvent =
      poolType === "独家频段" || poolType === "音擎频段";
    const noDeviationRate = isCharacterOrConeEvent
      ? calculateNoDeviationRateZzz(records)
      : null;
    const avgUp = isCharacterOrConeEvent
      ? calculateUpAverageZzz(pools[poolType])
      : null;
  ...
  GACHA_TYPE_ORDER.forEach((poolType) => {
    ...
    if (pools[poolType]) {
      ...
      const avgFiveStar = calculateDrawsBetweenZzz(pools[poolType], "S");
      const avgUp =
        poolType === "独家频段" || poolType === "音擎频段"
          ? calculateUpAverageZzz(pools[poolType])
          : null;

      const avgFiveStarText =
        typeof avgFiveStar === "number"
          ? `${avgFiveStar.toFixed(2)}`
          : avgFiveStar;
      const avgUpText =
        typeof avgUp === "number" ? `${avgUp.toFixed(2)}` : avgUp;

      const lastFiveStarDraws = calculateLastDrawsZzz(pools[poolType], "S");
      const lastFourStarDraws = calculateLastDrawsZzz(pools[poolType], "A");

      // "最非" 和 "最欧"
      const { maxDraws: mostDraws, minDraws: leastDraws } =
        calculateMostDrawsZzz(pools[poolType], "S");
      ...
      const progressBars = `
                ${generateProgressBar(
                  lastFiveStarDraws,
                  90,
                  "rgba(243, 213, 138,0.7)",
                  "距离上个S"
                )}
                ${generateProgressBar(
                  lastFourStarDraws,
                  10,
                  "rgba(214, 199, 255,0.7)",
                  "距离上个A"
                )}
            `;
  ...
  recordDisplay.addEventListener("wheel", (event) => {
    ...
    if (target) {
      if (
        ...
        (event.deltaY > 0 &&
          target.scrollTop + target.offsetHeight < target.scrollHeight) // 向下滚动
      ) {
        return;
      }
    }
