...
async function loadBackgroundSettings() {
  try {
    const rows = await new Promise((resolve, reject) => {
      db.all(
        'SELECT key, value FROM settings WHERE key IN ("backgroundImage", "backgroundURL", "backgroundOpacity")',
        [],
        (err, rows) => {
          if (err) {
            reject("加载背景设置失败:" + err);
...
async function loadBackground(mainWindow) {
  try {
    ...
    // 优先使用网络图片URL，如果没有则使用本地图片路径
    if (settings.backgroundURL) {
      // 使用网络图片URL
      mainWindow.webContents.executeJavaScript(`
                document.body.style.background = "linear-gradient(rgba(33, 33, 33, ${bgOpacity}), rgba(33, 33, 33, ${bgOpacity})), url('${settings.backgroundURL}')";
...
ipcMain.handle("restoreDefaultBackgroundSettings", async () => {
  try {
    ...
    const defaultBackgroundURL = null;
