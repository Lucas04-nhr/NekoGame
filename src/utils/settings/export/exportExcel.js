...
async function exportGachaDataToExcel(customPath = null) {
  let exportFolder, filePaths;

  if (customPath) {
    // 使用用户指定的路径
    exportFolder = path.dirname(customPath);
    const baseName = path.basename(customPath, path.extname(customPath));
    filePaths = {
      zh: customPath,
      en: path.join(exportFolder, `${baseName}_en.xlsx`),
    };
  } else {
    // 使用默认路径
    exportFolder = path.join(process.env.NEKO_GAME_FOLDER_PATH, "export");
...
ipcMain.handle("exportGachaData", async () => {
  const { canceled, filePath } = await dialog.showSaveDialog({
    title: "保存鸣潮抽卡记录",
    defaultPath: "wuwa_gacha_logs.xlsx",
  ...
  try {
    return await exportGachaDataToExcel(filePath);
  } catch (error) {
