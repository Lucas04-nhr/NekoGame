...
function generateSecretKey() {
    return crypto.randomBytes(32);  // 生成 32 字节的密钥
}
...
function encrypt(text, secretKey) {
    ...
    return { encryptedText: encrypted, iv: IV.toString('hex') };  // 返回加密文本和 IV
}

// 解密函数
function decrypt(encryptedText, secretKey, iv) {
    const ivBuffer = Buffer.from(iv, 'hex');  // 将 IV 从十六进制转换为 Buffer
...
function saveSyncConfigToFile(repoUrl, token) {
    ...
    if (!fs.existsSync(keyDirectory)) {
        fs.mkdirSync(keyDirectory);  // 确保存储文件夹存在
    }
    ...
    fs.writeFileSync(secretKeyFilePath, secretKey.toString('hex'));  // 保存为 hex 格式
...
function loadSyncConfigFromFile() {
    ...
    if (fs.existsSync(configFilePath) && fs.existsSync(secretKeyFilePath)) {
        ...
        const secretKey = Buffer.from(secretKeyHex, 'hex');  // 从 hex 转换为 Buffer
